    ...
    if session.get_checkpoint():
        if session.get_checkpoint():
            ...
            with loaded_checkpoint.as_directory() as loaded_checkpoint_dir:
                ...
                for i in range(len(model_list)):
                    model_list[i].load_state_dict(checkpoint[f"model_ens{i + 1}_state_dict"])
...
def pbt_hpo(subject, model, args, config_dict):
    print("Hyparparameter optimization")
    ...
    for i in range(1, args.num_models+1):
        lr_dict.update({f'ens{i}_lr':lambda: np.random.uniform(low=1e-05, high=1e-03)})
        wd_dict.update({f'ens{i}_wd': lambda: np.random.uniform(low=1e-05, high=1e-03)})
        lr_space_dict.update({f"ens{i}_lr": tune.uniform(1e-05, 1e-03)})
        wd_space_dict.update({f"ens{i}_wd": tune.uniform(1e-05, 1e-03)})
    ...
    # Initial architectures (fixed)
    for i in range(1, args.num_models+1):
        other_key_value.update({f'ens{i}_F1':config_dict[f'ens{i}_F1'],
                                f'ens{i}_T1':config_dict[f'ens{i}_T1'],
                                f'ens{i}_T2':config_dict[f'ens{i}_T2']})

    ## Set scheduler
    # parameter initial
    param_space_dict = {}
    param_space_dict.update(other_key_value)
    # tuning hyperparameter config
    param_space_dict.update(lr_space_dict)
    param_space_dict.update(wd_space_dict)
    # PBT: hyperparam_mutations
    hyperparam_dict = {}
    hyperparam_dict.update(lr_dict)
    hyperparam_dict.update(wd_dict)
